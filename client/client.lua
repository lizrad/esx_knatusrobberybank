...
AddEventHandler(
	...
	function()
		...
		TriggerServerEvent(
			"esx_holdupbank:opendoor",
			Banks[bank].hackposition.x,
			Banks[bank].hackposition.y,
			Banks[bank].hackposition.z,
			Banks[bank].doortype
		)
...
AddEventHandler(
	...
	function(x, y, z, doortype)
		...
		local coords = vector3(x, y, z) -- fix for vectors
		local key = GetHashKey(doortype)
		local filter = {}
		filter[key] = true
		local obs, distance = ESX.Game.GetClosestObject(coords, filter) -- instant open for people already in site
		...
		print(doortype, GetEntityHeading(obs), rotation)
		globalDoortype = doortype
		Citizen.CreateThread(
			function()
				while dooropen do
					Wait(2000)
					local obs, distance = ESX.Game.GetClosestObject(globalcoords, filter)
		...
		while true do
			...
			if holdingup then
				drawTxt(
					...
					1.0,
					0.4,
					_U("robbery_of") .. secondsRemaining .. _U("seconds_remaining"),
					255,
					255,
					255,
					255
